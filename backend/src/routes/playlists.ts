...
import { requireAuth, requireApproved, getAuth } from '../middleware/clerkAuth.js';
...
export function createPlaylistRoutes(tidalService: TidalService): Router {
   ...
   */
  router.post('/export', requireAuth, requireApproved, async (req: Request, res: Response) => {
    ...
    try {
      ...
      // 6. Create playlist
      const playlistId = await tidalService.createPlaylist(name, tidalTokens.accessToken);
      ...
      await tidalService.addTracksToPlaylist(playlistId, trackIds, tidalTokens.accessToken);
