...
export class LibraryService {
  ...
  async addAlbumToLibrary(
    ...
    try {
      ...
      if (existingAlbum) {
        logger.info("album_already_exists", {
          tidalAlbumId,
          userId,
      ...
      if (trackTidalIds.length > 0) {
        try {
          const isrcMap =
            await this.tidalService.batchFetchTrackIsrcs(trackTidalIds);
      ...
      libraryAlbum.releaseDate = albumData.releaseDate
  ...
  async addTrackToLibrary(
    ...
    try {
      ...
      if (existingTrack) {
        logger.info("track_already_exists", {
          tidalTrackId,
          userId,
  ...
  async getLibraryAlbums(userId: string): Promise<LibraryAlbum[]> {
    ...
    try {
      ...
      logger.info("get_library_albums_success", {
        userId,
        count: albums.length,
      });
      return albums;
  ...
  async getLibraryTracks(userId: string): Promise<LibraryTrack[]> {
    ...
    try {
      ...
      logger.info("get_library_tracks_success", {
        userId,
        count: tracks.length,
      });
      return tracks;
  ...
  async removeAlbumFromLibrary(id: string, userId: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error("remove_album_from_library_failed", {
        id,
        userId,
        error: String(error),
      });
  ...
  async removeTrackFromLibrary(id: string, userId: string): Promise<boolean> {
    ...
    try {
      ...
      logger.error("remove_track_from_library_failed", {
        id,
        userId,
        error: String(error),
      });
