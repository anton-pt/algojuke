...
} from "../../../src/services/agentTools/retry.js";
...
describe("getUserFriendlyMessage", () => {
  ...
  it("uses tool name in default message", () => {
    ...
    expect(getUserFriendlyMessage(error, "semanticSearch")).toBe(
      "Vector search is temporarily unavailable",
    );
    expect(getUserFriendlyMessage(error, "tidalSearch")).toBe(
      "Tidal search is temporarily unavailable",
    );
    expect(getUserFriendlyMessage(error, "batchMetadata")).toBe(
      "Metadata lookup is temporarily unavailable",
    );
    expect(getUserFriendlyMessage(error, "albumTracks")).toBe(
      "Album tracks is temporarily unavailable",
    );
  });
});
