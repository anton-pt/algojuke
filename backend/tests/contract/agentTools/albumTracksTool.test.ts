...
} from "../../../src/services/agentTools/albumTracksTool.js";
...
describe("executeAlbumTracks", () => {
  ...
  describe("input validation", () => {
    it("rejects empty album ID", async () => {
      ...
      await expect(
        executeAlbumTracks(input, mockContext),
  ...
  describe("album fetch", () => {
    it("fetches album metadata and tracks", async () => {
      ...
      expect(mockTidalService.getAlbumTrackListing).toHaveBeenCalledWith(
        "album-123",
      );
  ...
  describe("library status enrichment", () => {
    it("sets inLibrary=true for tracks in library", async () => {
      ...
      mockLibraryAlbumRepository.createQueryBuilder.mockReturnValue({
        ...
        getMany: vi.fn().mockResolvedValue([
          {
            trackListing: [{ isrc: "GBAYE9700001" }, { isrc: "GBAYE9700002" }],
          },
        ]),
      });
  ...
  describe("error handling", () => {
    it("throws non-retryable error for album not found", async () => {
      mockTidalService.getAlbumById.mockRejectedValue(
        new Error("Album not found on Tidal"),
      );
    ...
    it("handles ISRC fetch failure gracefully", async () => {
      mockTidalService.batchFetchTrackIsrcs.mockRejectedValue(
        new Error("ISRC fetch failed"),
      );
