/**
 * GCS Client Contract Tests (Backend Service)
 *
 * Tests the GCS signed URL client input validation schemas and error mapping.
...
describe("GCS Client Contract (Backend)", () => {
  ...
  describe("GetSignedUrlInputSchema", () => {
    it("should validate valid input with default expiry", () => {
      const input = {
        path: "mixes/abc123/seg001.mp3",
      };

      const result = GetSignedUrlInputSchema.safeParse(input);
      expect(result.success).toBe(true);
      if (result.success) {
    ...
    it("should validate valid input with custom expiry", () => {
      const input = {
        path: "mixes/abc123/seg001.mp3",
        expiryHours: 1,
      };
      ...
      expect(result.success).toBe(true);
      if (result.success) {
        expect(result.data.expiryHours).toBe(1);
      }
    });

    it("should validate max expiry hours (168)", () => {
      const input = {
        path: "mixes/abc123/seg001.mp3",
        expiryHours: 168,
      };
    ...
    it("should reject expiry hours less than 1", () => {
      const input = {
        path: "mixes/abc123/seg001.mp3",
        expiryHours: 0,
      };
    ...
    it("should reject expiry hours greater than 168", () => {
      const input = {
        path: "mixes/abc123/seg001.mp3",
        expiryHours: 169,
      };
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toContain("cannot exceed 168");
      }
    });
