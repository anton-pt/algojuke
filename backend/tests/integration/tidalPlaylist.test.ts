...
const hasTidalCredentials = process.env.TIDAL_CLIENT_ID && process.env.TIDAL_CLIENT_SECRET;
  ...
  describe('batchFetchTracksByIsrc', () => {
    ...
    it('should handle batch of 20+ ISRCs (chunking)', async () => {
      ...
      const isrcs = [
        ...
        ...Array.from({ length: 24 }, (_, i) => `TEST${i.toString().padStart(8, '0')}`),
      ];
...
describe('Playlist Export Endpoint (Mock)', () => {
  it('should validate request body structure', async () => {
    ...
    const { PlaylistExportRequestSchema } = await import('../../src/schemas/playlist.js');
  ...
  it('should reject invalid ISRC format', async () => {
    const invalidRequest = {
      name: 'My Playlist',
      tracks: [
        { isrc: 'INVALID', title: 'Track 1', artist: 'Artist 1' },
      ],
    };
