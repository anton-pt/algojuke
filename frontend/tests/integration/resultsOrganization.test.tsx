...
describe("Results Organization Integration Tests", () => {
  test("broad search shows albums and tracks in separate sections", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: /albums \(500\)/i }),
    ).toBeInTheDocument();

    // Verify Tracks section exists with heading
    expect(
      screen.getByRole("heading", { name: /tracks \(5000\)/i }),
    ).toBeInTheDocument();
  ...
  test("sections show correct result counts", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("heading", { name: /albums \(250\)/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("heading", { name: /tracks \(3000\)/i }),
      ).toBeInTheDocument();
    });
  });
  ...
  test("sections remain organized after multiple searches", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: /albums \(100\)/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: /tracks \(500\)/i }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("heading", { name: /albums \(200\)/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: /tracks \(1000\)/i }),
    ).toBeInTheDocument();
