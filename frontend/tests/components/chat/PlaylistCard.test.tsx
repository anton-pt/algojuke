...
} from "../../../src/components/chat/PlaylistCard";
...
const mockEnrichedTrack = {
  ...
  duration: 285,
  reasoning:
    "A beautiful ballad about lost love that fits the melancholic mood.",
  enriched: true,
  tidalId: "12345678",
};
...
describe("PlaylistCard", () => {
  describe("Title rendering (T024)", () => {
    ...
    it("renders long titles without truncation in header", () => {
      const longTitle =
        "A Very Long Playlist Title That Should Not Be Truncated In The Header Section";
  ...
  describe("Mixed enrichment states", () => {
    ...
    it("visually differentiates unenriched tracks", () => {
      ...
      const trackRow = screen
        .getByText("Underground Hit")
        .closest(".playlist-card__track");
  ...
  describe("Accordion expand/collapse (T030)", () => {
    it("reasoning is hidden by default", () => {
      ...
      expect(
        screen.queryByText(mockEnrichedTrack.reasoning),
    ...
    it("clicking a track expands to show reasoning", () => {
      ...
      const firstTrack = screen
        .getByText("Someone Like You")
        .closest(".playlist-card__track");
    ...
    it("only one track can be expanded at a time (single expansion mode)", () => {
      const tracksWithDifferentReasoning = [
        {
          ...mockEnrichedTrack,
          isrc: "TRACK1111111",
          reasoning: "First track reasoning",
        },
        {
          ...mockEnrichedTrack,
          isrc: "TRACK2222222",
          title: "Second Track",
          reasoning: "Second track reasoning",
        },
      ];
      render(
        <PlaylistCard
          {...defaultProps}
          tracks={tracksWithDifferentReasoning}
        />,
      );
      ...
      const secondTrack = screen
        .getByText("Second Track")
        .closest(".playlist-card__track");
      fireEvent.click(secondTrack!);
      expect(
        screen.queryByText("First track reasoning"),
      ).not.toBeInTheDocument();
  ...
  describe("Save to Tidal button (T015-T020)", () => {
    it("renders Save to Tidal button", () => {
      ...
      expect(
        screen.getByRole("button", { name: /save.*tidal/i }),
      ).toBeInTheDocument();
    });
  ...
  describe("Disabled button when no Tidal connection (T017-T018)", () => {
    ...
    it("shows tooltip explaining why button is disabled", () => {
      ...
      expect(button).toHaveAttribute(
        "title",
        expect.stringMatching(/connect.*tidal/i),
      );
    });
