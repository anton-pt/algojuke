...
import { SavePlaylistModal, type SavePlaylistModalProps } from '../../../src/components/chat/SavePlaylistModal';
...
describe('SavePlaylistModal', () => {
  ...
  describe('Cancel action (T024-T026)', () => {
    ...
    it('resets name field to default when reopened', async () => {
      ...
      const reopenedInput = screen.getByRole('textbox', { name: /playlist name/i });
  ...
  describe('Loading state (T032-T034)', () => {
    ...
    it('disables name input when loading', () => {
      render(<SavePlaylistModal {...defaultProps} isLoading={true} />);
      expect(screen.getByRole('textbox', { name: /playlist name/i })).toBeDisabled();
    });
    ...
    it('shows warning when cancel clicked during loading (FR-097)', () => {
      ...
      expect(screen.getByRole('button', { name: /keep saving/i })).toBeInTheDocument();
    ...
    it('dismisses warning when "Keep saving" clicked', () => {
      ...
      expect(screen.queryByText(/save is in progress/i)).not.toBeInTheDocument();
    });

    it('calls onCancel when "Cancel anyway" clicked', () => {
      const onCancel = vi.fn();
      render(<SavePlaylistModal {...defaultProps} isLoading={true} onCancel={onCancel} />);
  ...
  describe('Error handling (T027-T031)', () => {
    it('displays error message when error prop is set', () => {
      render(<SavePlaylistModal {...defaultProps} error="Failed to create playlist" />);
      expect(screen.getByText(/failed to create playlist/i)).toBeInTheDocument();
    });
