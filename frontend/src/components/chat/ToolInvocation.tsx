...
function StatusBadge({
  ...
  const getLabel = () => {
    switch (status) {
      case "executing":
        ...
        return toolName === "suggestPlaylist"
          ? "Building playlist..."
          : "Searching...";
      case "completed":
        return "Done";
  ...
  return (
    <span
      className={`tool-invocation__status tool-invocation__status--${status}`}
    >
...
function ToolResultsRenderer({
  ...
  if (!output || typeof output !== "object") {
    return (
      <div className="tool-invocation__results-empty">No results available</div>
    );
  }
    ...
    return (
      <div className="tool-invocation__results-list">
        {data.tracks.map((track: Record<string, unknown>, index: number) => (
          <div
            key={String(track.isrc || index)}
            className="tool-invocation__result-item"
          >
    ...
    return (
      <div className="tool-invocation__results-list">
        {data.albums.map((album: Record<string, unknown>, index: number) => (
          <div
            key={String(album.tidalId || index)}
            className="tool-invocation__result-item"
          >
            <span className="tool-invocation__result-title">
...
export function ToolInvocation({
  ...
  const handleSaveClick = useCallback(
    (title: string, tracks: TrackForExport[]) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="tool-invocation__name">
        ...
        {durationMs !== undefined && status !== "executing" && (
          <span className="tool-invocation__duration">
      ...
      {error ? <div className="tool-invocation__error">{error}</div> : null}

      {showResults ? (
        <div
          className={`tool-invocation__results ${isPlaylist ? "tool-invocation__results--playlist" : ""}`}
        >
          <ToolResultsRenderer
            output={output}
            toolName={toolName}
