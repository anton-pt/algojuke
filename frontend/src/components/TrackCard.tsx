...
export function TrackCard({ track }: TrackCardProps) {
  ...
  const { data: libraryData } = useQuery<{
    getLibraryTracks: Array<{ tidalTrackId: string }>;
  }>(GET_LIBRARY_TRACKS, { fetchPolicy: "cache-first" });

  const [addTrackToLibrary, { loading: addLoading }] = useMutation(
    ADD_TRACK_TO_LIBRARY,
    {
      ...
      onCompleted: (data) => {
        if (data.addTrackToLibrary.__typename === "LibraryTrack") {
          ...
          data.addTrackToLibrary.__typename === "DuplicateLibraryItemError"
        ) {
          setAddError("Track already in library");
          setTimeout(() => setAddError(null), 3000);
        } else if (
          data.addTrackToLibrary.__typename === "TidalApiUnavailableError"
        ) {
